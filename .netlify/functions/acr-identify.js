exports.handler = async (event) => {
  try {
    ...
    const key = await crypto.subtle.importKey("raw", enc.encode(ACRCLOUD_ACCESS_SECRET), { name: "HMAC", hash: "SHA-1" }, false, ["sign"]);
    ...
    const r = await fetch(`https://${ACRCLOUD_HOST}/v1/identify`, {
      method: "POST",
      headers: { "Content-Type": contentType, "Content-Length": String(contentLength), Authorization: auth, Date: now },
      body: buf
    });

    const json = await r.json();
    if (!json || json.status?.code !== 0 || !json.metadata) return { statusCode: 200, body: JSON.stringify(null) };
